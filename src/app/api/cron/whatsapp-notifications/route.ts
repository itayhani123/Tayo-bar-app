...
type NotificationRow = { id: string; notification_type: WhatsAppNotificationType; payload: NotificationContext };

export async function POST(request: Request) {
  if (!hasValidCronAuthorization(request.headers.get("authorization"), process.env.CRON_SECRET)) return Response.json({ error: "Unauthorized" }, { status: 401 });
  const settings = await getWhatsAppSettings();
  if (!settings.enabled) return Response.json({ processed: 0, disabled: true });
  const admin = createAdminClient();
  const { data, error } = await admin.from("whatsapp_notifications").select("id, notification_type, payload").eq("status", "pending").lte("scheduled_for", new Date().toISOString()).order("scheduled_for").limit(20);
