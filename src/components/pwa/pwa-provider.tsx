...
export function PwaProvider() {
  ...
  return <>{isOffline && <div role="status" className="fixed inset-x-0 top-0 z-[100] bg-amber-600 px-4 py-2 text-center text-sm font-medium text-white">אין חיבור לאינטרנט — פעולות שמירה אינן זמינות</div>}{waitingWorker && <aside className="fixed inset-x-4 bottom-[max(1rem,env(safe-area-inset-bottom))] z-[90] mx-auto flex max-w-md items-center justify-between gap-3 rounded-xl bg-black p-3 text-sm text-white shadow-xl"><span>גרסה חדשה זמינה</span><button type="button" onClick={applyUpdate} className="rounded-lg border border-white bg-white px-3 py-2 font-medium text-black">רענן עכשיו</button></aside>}{(installPrompt || showIosHelp) && !waitingWorker && <aside className="fixed inset-x-4 bottom-[max(1rem,env(safe-area-inset-bottom))] z-[80] mx-auto max-w-md rounded-2xl border border-slate-200 bg-white p-4 text-right shadow-xl"><div className="flex items-start justify-between gap-4"><div className="flex min-w-0 items-start gap-3"><Image src="/logo.png" alt="TAYO" width={550} height={303} className="mt-1 h-auto w-20 shrink-0" /><div><h2 className="font-semibold text-slate-950">התקנת TAYO</h2><p className="mt-1 text-sm leading-6 text-slate-600">{showIosHelp ? "להתקנת האפליקציה באייפון: לחץ על כפתור השיתוף ובחר ״הוסף למסך הבית״." : "התקינו את המערכת לגישה מהירה ממסך הבית."}</p></div></div><button type="button" aria-label="סגור הצעת התקנה" onClick={dismissInstall} className="text-xl leading-none text-slate-500">×</button></div>{installPrompt && <button type="button" onClick={install} className="mt-3 rounded-lg bg-black px-4 py-2 text-sm font-medium text-white hover:bg-slate-800">התקן אפליקציה</button>}</aside>}</>;
}
