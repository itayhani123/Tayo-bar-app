...
export function Sidebar({ isOpen, onClose, role }: SidebarProps) {
  ...
  return (
    <>
      ...
      <button aria-label="סגירת ניווט" onClick={onClose} className={`fixed inset-0 z-30 bg-slate-950/35 transition-opacity lg:hidden ${isOpen ? "opacity-100" : "pointer-events-none opacity-0"}`} />
      <aside className={`fixed inset-y-0 right-0 z-40 flex w-[272px] translate-x-full flex-col border-l border-slate-200 bg-white px-4 py-5 transition-transform duration-200 lg:translate-x-0 ${isOpen ? "translate-x-0" : ""}`}>
        ...
        <div className="rounded-xl bg-slate-50 p-3">
          ...
          <p className="mt-1 text-xs leading-5 text-slate-500">מרכז התפעול שלך.</p>
        </div>
      </aside>
    </>
  );
