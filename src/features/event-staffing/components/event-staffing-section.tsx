...
import { calculateEstimatedAfterAdvance, calculateIncomeTaxAdvance, calculateOperationalProfit, calculateVat } from "@/lib/finance/calculations";
...
export function EventStaffingSection({ eventId, eventDate, guestCount, pricePerGuest, vatRate, priceIncludesVat, estimatedAlcoholCost, showFinancials = true, onSuccess, onError }: { eventId: string; eventDate: string; guestCount: number; pricePerGuest: number; vatRate: number; priceIncludesVat: boolean; estimatedAlcoholCost: number; showFinancials?: boolean; onSuccess: (message: string) => void; onError: (message: string) => void }) {
  const assignments = useEventAssignments(eventId, showFinancials);
  const employees = useEmployees();
  const incomeTaxRate = useIncomeTaxAdvanceRate(showFinancials);
  ...
  const revenue = calculateVat({ guestCount, pricePerGuest, vatRate, priceIncludesVat });
  const staffCost = assigned.reduce((total, assignment) => total + (getAssignmentSalary(assignment) ?? 0), 0);
  const operationalProfit = calculateOperationalProfit(revenue.netRevenue, staffCost, estimatedAlcoholCost);
  const advanceRate = incomeTaxRate.data ?? 3.2;
  const taxAdvance = calculateIncomeTaxAdvance(revenue.netRevenue, advanceRate);
  const afterAdvance = calculateEstimatedAfterAdvance(operationalProfit, taxAdvance);
  ...
  return <section className="border-t border-border pt-7"><div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div><h3 className="text-sm font-semibold">צוות האירוע</h3><div className="mt-2 flex flex-wrap gap-x-6 gap-y-1 text-sm text-muted-foreground"><span>ברמנים מומלצים: <strong className="text-foreground">{recommended}</strong></span><span>משובצים בפועל: <strong className="text-foreground">{assigned.length}</strong></span></div></div><Button type="button" variant="outline" onClick={() => setDialog("create")} disabled={employees.isLoading || !availableEmployees.length}><Plus />הוסף עובד</Button></div>
    {showFinancials && <div className="mt-5 rounded-lg bg-muted/50 p-4"><div className="grid grid-cols-2 gap-3 text-sm sm:grid-cols-4"><Summary label="הכנסה לפני מע״מ" value={formatMoney(revenue.netRevenue)} /><Summary label="מע״מ" value={formatMoney(revenue.vatAmount)} /><Summary label="הכנסה כולל מע״מ" value={formatMoney(revenue.grossRevenue)} /><Summary label="עלות עובדים" value={formatMoney(staffCost)} /><Summary label="עלות אלכוהול" value={formatMoney(estimatedAlcoholCost)} /><Summary label="רווח תפעולי לפני מס" value={formatMoney(operationalProfit)} /><Summary label={`מקדמת מס הכנסה משוערת (${advanceRate}%)`} value={formatMoney(taxAdvance)} /><Summary label="יתרה משוערת לאחר מקדמה" value={formatMoney(afterAdvance)} /></div><p className="mt-4 text-xs text-muted-foreground">החישוב הוא הערכה ניהולית ואינו תחליף לחישוב רואה חשבון.</p></div>}
    {assignments.isLoading || employees.isLoading ? <State><LoaderCircle className="size-4 animate-spin" />טוען את צוות האירוע...</State> : assignments.isError || employees.isError ? <div className="mt-5 rounded-lg bg-red-50 p-4 text-sm text-red-700">לא ניתן לטעון את צוות האירוע. נסו שוב.</div> : assigned.length ? <div className="mt-5 overflow-hidden rounded-lg border border-border"><div className="divide-y divide-border">{assigned.map((assignment) => <AssignmentRow key={assignment.id} assignment={assignment} showFinancials={showFinancials} onEdit={setDialog} onRemove={setRemoving} />)}</div></div> : <State><Users className="size-4" />אין עובדים משובצים</State>}
