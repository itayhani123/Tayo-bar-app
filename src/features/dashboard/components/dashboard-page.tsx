...
import { actualPaymentStatus, assignmentSalary, buildWarnings, eventReadiness, eventStaffCost, recommendedBartenders } from "../utils/dashboard-calculations";
...
export function DashboardPage({ access }: { access: DashboardAccess }) {
  ...
  const query = useDashboard(month, access === "owner");
  ...
  return <div className="mx-auto max-w-7xl space-y-6"><header className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between"><div><p className="text-sm font-medium text-muted-foreground">תפעול</p><h2 className="mt-1 text-2xl font-semibold">מרכז בקרה</h2><p className="mt-2 text-sm text-muted-foreground">תמונת מצב חודשית ופעילות קרובה.</p></div><MonthPicker month={month} onChange={setMonth} /></header>
    {access === "owner" && <><div className="grid grid-cols-2 gap-3 lg:grid-cols-4"><Kpi label="הכנסות בפועל החודש לפני מע״מ" value={formatMoney(netRevenue)} /><Kpi label="מע״מ עסקאות בפועל" value={formatMoney(vatAmount)} /><Kpi label="הכנסות בפועל החודש כולל מע״מ" value={formatMoney(grossRevenue)} /><Kpi label="מקדמות מס הכנסה משוערות" value={formatMoney(taxAdvances)} /><Kpi label="עלות עובדים בפועל" value={formatMoney(staffCost)} /><Kpi label="עלות אלכוהול בפועל" value={formatMoney(alcoholCost)} /><Kpi label="רווח בפועל החודש לפני מס" value={formatMoney(operationalProfit)} /><Kpi label="יתרה לאחר מקדמות" value={formatMoney(estimatedAfterAdvances)} /></div><MonthlyBreakdown revenue={netRevenue} staff={staffCost} alcohol={alcoholCost} profit={operationalProfit} /></>}
...
function MonthlyBreakdown({ revenue, staff, alcohol, profit }: { revenue: number; staff: number; alcohol: number; profit: number }) { const max = Math.max(revenue, staff, alcohol, Math.abs(profit), 1); return <section className="rounded-xl border border-border bg-card p-5 shadow-sm"><h3 className="font-semibold">פירוט בפועל לחודש</h3><div className="mt-4 grid gap-4 sm:grid-cols-2">{[["הכנסה בפועל לפני מע״מ", revenue, "bg-indigo-500"], ["עלות עובדים בפועל", staff, "bg-amber-500"], ["עלות אלכוהול בפועל", alcohol, "bg-orange-500"], ["רווח בפועל", profit, profit >= 0 ? "bg-emerald-500" : "bg-red-500"]].map(([label, value, color]) => <div key={String(label)}><div className="flex justify-between text-sm"><span>{label}</span><span>{formatMoney(Number(value))}</span></div><div className="mt-2 h-2 overflow-hidden rounded-full bg-muted"><div className={`h-full rounded-full ${color}`} style={{ width: `${Math.min(100, Math.abs(Number(value)) / max * 100)}%` }} /></div></div>)}</div></section>; }
