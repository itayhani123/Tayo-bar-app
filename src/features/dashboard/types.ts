...
export type DashboardEvent = { id: string; eventDate: string; startTime: string; venueName: string; eventType: string; clientName: string; guestCount: number; pricePerGuest: number; vatRate: number; priceIncludesVat: boolean; estimatedAlcoholCost: number; paymentStatus: PaymentStatus; securityCheckReceived: boolean; invoiceIssued: boolean; createdAt: string; updatedAt: string; assignments: DashboardAssignment[] };
