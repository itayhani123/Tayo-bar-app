...
export function EventFormDialog({ event, venues, eventTypes, packages, onClose, onSuccess, onError }: Props) {
  ...
  </form>{event && <div className="px-5 pb-6"><EventStaffingSection eventId={event.id} eventDate={watch("eventDate")} guestCount={watch("guestCount") || 0} pricePerGuest={watch("pricePerGuest") || 0} estimatedAlcoholCost={watch("estimatedAlcoholCost") || 0} onSuccess={onSuccess} onError={onError} /></div>}</div></div>;
}
