...
type Props = { events: EventRecord[]; showFinancials?: boolean; onNew: () => void; onEdit: (event: EventRecord) => void; onDelete: (event: EventRecord) => void };
...
export function EventsTable({ events, showFinancials = true, onNew, onEdit, onDelete }: Props) {
  ...
  const columns: ColumnDef<EventRecord>[] = [
    ...
    ...(showFinancials ? [{ accessorKey: "paymentStatus", header: "סטטוס תשלום", cell: ({ row }: { row: { original: EventRecord } }) => <Status event={row.original} /> }, { id: "revenue", header: "הכנסה צפויה", cell: ({ row }: { row: { original: EventRecord } }) => formatMoney(revenue(row.original)) }] as ColumnDef<EventRecord>[] : []),
  ...
  return <section data-show-financials={showFinancials} className="overflow-hidden rounded-xl border border-border bg-card shadow-sm">
